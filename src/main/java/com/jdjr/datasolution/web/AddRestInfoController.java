...
@Controller
@RequestMapping("/dp")
public class AddRestInfoController {

    @Resource
    ISaveDpService saveDataService;

    //插入月评论数
    @RequestMapping("/addMonthComments")
    public void addMonthComments(List<DPShopDO> dpShopDO, int id) {
    ...
    /*//插入经度和纬度
    @RequestMapping("/addLatAndLgn")
    public void addLatAndLgn() throws Exception {
                ...
                if (httpsConn != null) {
                    ...
                    while ((data = br.readLine()) != null) {
                        if (count == 5) {
                            ...
                            dpShopDO.setLongitude(lng);
                            count++;
                        } else if (count == 6) {
                            lat = data.substring(data.indexOf(":") + 1);//纬度
                            dpShopDO.setLatitude(lat);
                            count++;
                        } else {
                            count++;
                        }
                    }
                    saveDataService.addLngAndLat(dpShopDO.getLongitude(), dpShopDO.getLatitude(), i);
                }
            } catch (IOException e) {
